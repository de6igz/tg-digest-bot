...
// Client — минимальный интерфейс клиента Точки, который нам нужен.
...
func NewService(b domain.Billing, client Client, notificationURL string, log zerolog.Logger) *Service {
	return &Service{
		billing:          b,
		client:           client,
		defaultNotifyURL: notificationURL,
		log:              log,
	}
}

// CreateInvoiceWithQRCode
// 1) гарантирует наличие аккаунта пользователя,
// 2) регистрирует QR в Точке,
// 3) создаёт Invoice в нашей БД c метаданными SBP.
func (s *Service) CreateInvoiceWithQRCode(ctx context.Context, params CreateInvoiceParams) (CreateInvoiceResult, error) {
	...
	if params.RedirectUrl == "" {
		params.RedirectUrl = s.defaultNotifyURL
	}

	// Идемпотентный повтор — если уже есть Invoice с таким ключом, вернём его и связанные QR-метаданные (если есть).
	existing, err := s.billing.GetInvoiceByIdempotencyKey(ctx, params.IdempotencyKey)
	if err == nil {
		if sbpMeta, ok := domain.ExtractInvoiceSBPMetadata(existing.Metadata); ok {
	...
	// Сохраняем метаданные по SBP в инвойс
	sbpMeta := domain.InvoiceSBPMetadata{
		...
		Extra: map[string]any{
			"notification":    params.RedirectUrl,
...
// HandleIncomingPayment — обработка вебхука Точки: фиксируем входящий платёж.
